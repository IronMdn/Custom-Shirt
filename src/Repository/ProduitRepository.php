...
class ProduitRepository extends RepositoryAbstract
{
    ...
    public function findAllforAdmin()//recupérer tous les produits pour les afficher en back-end pour admin
    {
        $query = <<<EOS
...
EOS;
    ...
    public function findAll()//affichage des produits coté user
    {
        $query = <<<EOS
...
EOS;
    ...
    public function findById($id)//affichage d'un seul produit selon son id
    {
        $query = <<<EOS
...
EOS;
    ...
    public function save(Produit $produit)//cette methode enregistre un nouveau produit ou met à jour un produit existant
        //selon la valeur de la variable $where
    {
        $data = [
     ...
     * @return Article
     */
    public function buildFromArray(array $dbProduit)//création des objets pour les besoin d'affichage
    {
        $produit = new Produit();
